...
#INCLUDE REPOSITORY
from src.repository import apifunctions
...
def chatbot_message(messagedata: MessageApi, id_interaction, idrow, promp1):
    ...
    #prompt_final = promp1 + """ Utilice las siguientes piezas de contexto para responder la pregunta al final. Si no sabe la respuesta, simplemente diga que no tiene la información, no intente inventar una respuesta. No haga referencia a que está utilizando un texto.  Responda entregando la mayor cantidad de información posible."""
    prompt_final = """ Utilice las siguientes piezas de contexto para responder la pregunta al final. Si no sabe la respuesta, simplemente diga que no tiene la información, no intente inventar una respuesta. No haga referencia a que está utilizando un texto.  Responda entregando la mayor cantidad de información posible."""
    ...
    mycursor.execute("""SELECT function_name FROM iar2_functions WHERE identerprise = '%d'""" % (idempresa))
    function_names  = [row[0] for row in mycursor.fetchall()]    

    # Obtener referencias a las funciones
    tools = [getattr(apifunctions, func_name) for func_name in function_names]

    #print(tools)
    #tools = [apifunctions.get_indicador_value]

    functions = [format_tool_to_openai_function(f) for f in tools]
    model = ChatOpenAI(model_name=llm_name, temperature=0).bind(functions=functions)
...
def send_message(messagedata: MessageApi):
    ...
    mycursor.execute("""SELECT      id
                                    ...
                                    , CASE WHEN time (NOW()) > time_max THEN 1
                                      ...
                                      END fuera_time_max   
                                    , whatsappapi                          
                        FROM iar2_empresas WHERE typechatbot = 'API' AND codempresa = '%s'""" % (messagedata.enterprise))
